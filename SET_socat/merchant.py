import socket
import hashlib
import hmac
import base64
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad, unpad
from cryptography import x509
from cryptography.x509.oid import NameOID
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.asymmetric import rsa, padding as asym_padding
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives.asymmetric import dh
from cryptography.hazmat.primitives import serialization
import datetime
import os
import time
print("\033[1;32m---------商家服务器---------\033[0m")

def generate_certificate(subject_name):
    # 生成私钥
    private_key = rsa.generate_private_key(
        public_exponent=65537,
        key_size=2048,
        backend=default_backend()
    )

    # 生成证书主题和颁发者
    subject = x509.Name([
        x509.NameAttribute(NameOID.COUNTRY_NAME, u"US"),
        x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, u"California"),
        x509.NameAttribute(NameOID.LOCALITY_NAME, u"San Francisco"),
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"My Company"),
        x509.NameAttribute(NameOID.COMMON_NAME, subject_name),
    ])

    issuer = subject  # 自签名证书，颁发者是自己

    # 生成证书
    cert = x509.CertificateBuilder().subject_name(
        subject
    ).issuer_name(
        issuer
    ).public_key(
        private_key.public_key()
    ).serial_number(
        x509.random_serial_number()
    ).not_valid_before(
        datetime.datetime.now(datetime.timezone.utc)
    ).not_valid_after(
        # 证书有效期为1年
        datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=365)
    ).add_extension(
        x509.SubjectAlternativeName([x509.DNSName(u"localhost")]),
        critical=False,
    ).sign(private_key, hashes.SHA256(), default_backend())

    # 返回私钥和证书
    private_key_pem = private_key.private_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PrivateFormat.PKCS8,
        encryption_algorithm=serialization.NoEncryption()
    )

    cert_pem = cert.public_bytes(serialization.Encoding.PEM)
    
    return private_key_pem.decode('utf-8'), cert_pem.decode('utf-8')

def sign_message(private_key_pem, message):
    private_key = serialization.load_pem_private_key(
        private_key_pem.encode(),
        password=None,
        backend=default_backend()
    )
    signature = private_key.sign(
        message.encode(),
        asym_padding.PSS(
            mgf=asym_padding.MGF1(hashes.SHA256()),
            salt_length=asym_padding.PSS.MAX_LENGTH
        ),
        hashes.SHA256()
    )
    return base64.b64encode(signature).decode('utf-8')

def hash_message(message):
    hashed = hashlib.sha256(message.encode()).hexdigest()
    return hashed

def decrypt_rsa(private_key_pem, ciphertext):
    # 加载私钥
    private_key = serialization.load_pem_private_key(
        private_key_pem,
        password=None,
        backend=default_backend()
    )
    # 解密数据
    plaintext = private_key.decrypt(
        ciphertext,
        asym_padding.OAEP(
            mgf=asym_padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(),
            label=None
        )
    )
    return plaintext

def extract_public_key_from_cert(cert_pem):
    cert = x509.load_pem_x509_certificate(cert_pem.encode(), default_backend())
    public_key = cert.public_key()
    public_key_pem = public_key.public_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PublicFormat.SubjectPublicKeyInfo
    )
    return public_key_pem.decode('utf-8')

def verify_signature(public_key_pem, message, signature):
    public_key = serialization.load_pem_public_key(
        public_key_pem.encode(),
        backend=default_backend()
    )
    try:
        public_key.verify(
            base64.b64decode(signature),
            message.encode(),
            asym_padding.PSS(
                mgf=asym_padding.MGF1(hashes.SHA256()),
                salt_length=asym_padding.PSS.MAX_LENGTH
            ),
            hashes.SHA256()
        )
        return True
    except Exception as e:
        return False

def generate_temp_aes_key():
    aes_key = hashlib.sha256(os.urandom(32)).digest()[:16]  # 生成随机的32字节并截取16字节作为AES密钥
    return aes_key

def encrypt_message(pk, message):
    cipher = AES.new(pk, AES.MODE_CBC)
    ct_bytes = cipher.encrypt(pad(message, AES.block_size))
    iv = base64.b64encode(cipher.iv).decode('utf-8')
    ct = base64.b64encode(ct_bytes).decode('utf-8')
    return iv + ct

def decrypt_message(pk, encrypted_message):
    iv = base64.b64decode(encrypted_message[:24])
    ct = base64.b64decode(encrypted_message[24:])
    cipher = AES.new(pk, AES.MODE_CBC, iv)
    pt = unpad(cipher.decrypt(ct), AES.block_size)
    return pt

def encrypt_rsa(public_key_pem, plaintext):
    # 加载公钥
    public_key = serialization.load_pem_public_key(
        public_key_pem,
        backend=default_backend()
    )
    # 加密数据
    ciphertext = public_key.encrypt(
        plaintext,
        asym_padding.OAEP(
            mgf=asym_padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(),
            label=None
        )
    )
    return ciphertext

#定义节点1的IP地址和端口号
HOST1 = 'localhost'
PORT1 = 5000
# 生成商人的私钥和证书
privte_key_c='''-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----'''
cert_c='''-----BEGIN CERTIFICATE-----

BdkezHmFOINRw41Oy2uWrCVa
-----END CERTIFICATE-----'''
public_key_c=extract_public_key_from_cert(cert_c)
private_key_p='''-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----'''
cert_p='''-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'''
public_key_p=extract_public_key_from_cert(cert_p)
private_key_m='''-----BEGIN PRIVATE KEY-----

jptlsxt+DCDDu3m/9xO0ynyp
-----END PRIVATE KEY-----'''
cert_m='''-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'''
public_key_m=extract_public_key_from_cert(cert_m)

# 创建socket对象
server_socket1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket1.bind((HOST1, PORT1))
server_socket1.listen(5)

print("等待持卡人连接...")
# 等待节点2连接
client_socket, client_addr = server_socket1.accept()
print(f"持卡人连接成功！")
start=time.time()
# 等待购买请求
data = client_socket.recv(1024)
print(f"商家收到购买请求: {data.decode()}")
print()
# 发送回应给持卡人
response_to_cardholder = "Response to purchase request from Merchant"
print("\033[1;35mstep2:回应购买请求\033[0m")
print(response_to_cardholder)
client_socket.sendall(response_to_cardholder.encode())

# 商家发送证书给持卡人
print(f"商家发送证书")
client_socket.sendall(cert_m.encode())

#商家发送签名给持卡人
sig1=sign_message(private_key_m,hash_message(response_to_cardholder))
print(f"商家发送签名")
client_socket.sendall(sig1.encode())


#商家收到OI和PI的签名
sig2=client_socket.recv(500)
print(f"商家收到OI和PI的签名")
#print(sig2.decode())



recv1=client_socket.recv(2048)
print(f"商家收到加密后的OI")
OI=decrypt_rsa(private_key_m.encode(),recv1).decode()
print(f"商家解密OI: {OI}")


#商家接收H(PI)
recv2=client_socket.recv(64)

#商家接收证书
cert_c = client_socket.recv(1248)
print(f"商家收到持卡人的证书")

#商家验证OI和PI的签名

public_key_c=extract_public_key_from_cert(cert_c.decode())
print(f"获取持卡人的公钥")
print("\033[1;35mstep5:验证订单和支付信息\033[0m")
if verify_signature(public_key_c,hash_message(hash_message(OI)+recv2.decode()),sig2.decode()):
    print("持卡人的签名验证成功")
else:
    print("持卡人的签名验证失败")


recv3=client_socket.recv(256)
recv4=client_socket.recv(600)

#server_socket1.close()
HOST2 = 'localhost'
PORT2 = 5001
client_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
client_socket2.connect((HOST2, PORT2))
print("商家连接到支付网关")
print("\033[1;35mstep6:请求支付授权\033[0m")
AuthREQ = "Payment authorization request"
print("AuthREQ:",AuthREQ)
print()
tmp_key2=generate_temp_aes_key()
#print(tmp_key2)
sd1=encrypt_message(tmp_key2,public_key_p.encode())
# print(sd1)
# print(len(sd1))
sig=sign_message(private_key_m,hash_message(AuthREQ)+AuthREQ)
sd2=encrypt_message(tmp_key2,sig.encode())
# print(sd2)
# print(len(sd2))
# print(recv3)
# print(recv4)
client_socket2.sendall(sd1.encode())
client_socket2.sendall(sd2.encode())
client_socket2.sendall(recv3)
client_socket2.sendall(recv4)

sd1=client_socket2.recv(256)
enc1=client_socket2.recv(516)
sd2=client_socket2.recv(256)
enc2=client_socket2.recv(516)
#解密出sk_3
sk_3=decrypt_rsa(private_key_m.encode(),sd1)
print("解密出sk_3",sk_3)
dec1=decrypt_message(sk_3,enc1)
AuthRES=dec1.decode()[-18:]
print("得到AuthRES:",AuthRES)
print("\033[1;35mstep10:验证支付授权回应\033[0m")
sig3=dec1.decode()[:-18]
if verify_signature(public_key_p,hash_message(AuthRES),sig3):
    print("验证成功")
else:
    print("验证失败")
    
#向持卡人回应订单并发货
print("\033[1;35mstep11:回应订单并发货\033[0m")
RES2="Order confirmed and goods shipped"
print("RES2:",RES2)
print()
sig4=sign_message(private_key_m,hash_message(RES2))
client_socket.sendall(RES2.encode())
#商家发送请款请求
print("\033[1;35mstep13:发送支付请款请求\033[0m")
CapREQ="Request for payment"
print("CapREQ:",CapREQ)
print()
sk_5=generate_temp_aes_key()
sd5=encrypt_rsa(public_key_p.encode(),sk_5)
sig5=sign_message(private_key_m,hash_message(CapREQ))+CapREQ
enc5=encrypt_message(sk_5,sig5.encode())
client_socket2.sendall(sd5)
client_socket2.sendall(enc5.encode())

#商家接收支付网关的回应

sd6=client_socket2.recv(256)
enc6=client_socket2.recv(516)

#解密出sk_6
sk_6=decrypt_rsa(private_key_m.encode(),sd6)
print("解密出sk_6",sk_6)
dec6=decrypt_message(sk_6,enc6)
PayRES=dec6.decode()[-15:]
print("得到PayRES:",PayRES)
sig6=dec6.decode()[:-15]
print("\033[1;35mstep17:验证支付请款回应\033[0m")
if verify_signature(public_key_p,hash_message(PayRES),sig6):
    print("验证成功")
else:
    print("验证失败")

end=time.time()
#print(f"总耗时:{end-start}")