import socket
import hashlib
import hmac
import base64
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad, unpad
from cryptography import x509
from cryptography.x509.oid import NameOID
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.asymmetric import rsa, padding as asym_padding
from cryptography.hazmat.primitives import serialization
from cryptography.hazmat.backends import default_backend
from cryptography.hazmat.primitives.asymmetric import dh
from cryptography.hazmat.primitives import serialization
import datetime
import os
import time
print("\033[1;32m---------支付网关服务器---------\033[0m")

def generate_certificate(subject_name):
    # 生成私钥
    private_key = rsa.generate_private_key(
        public_exponent=65537,
        key_size=2048,
        backend=default_backend()
    )

    # 生成证书主题和颁发者
    subject = x509.Name([
        x509.NameAttribute(NameOID.COUNTRY_NAME, u"US"),
        x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, u"California"),
        x509.NameAttribute(NameOID.LOCALITY_NAME, u"San Francisco"),
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"My Company"),
        x509.NameAttribute(NameOID.COMMON_NAME, subject_name),
    ])

    issuer = subject  # 自签名证书，颁发者是自己

    # 生成证书
    cert = x509.CertificateBuilder().subject_name(
        subject
    ).issuer_name(
        issuer
    ).public_key(
        private_key.public_key()
    ).serial_number(
        x509.random_serial_number()
    ).not_valid_before(
        datetime.datetime.now(datetime.timezone.utc)
    ).not_valid_after(
        # 证书有效期为1年
        datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=365)
    ).add_extension(
        x509.SubjectAlternativeName([x509.DNSName(u"localhost")]),
        critical=False,
    ).sign(private_key, hashes.SHA256(), default_backend())

    # 返回私钥和证书
    private_key_pem = private_key.private_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PrivateFormat.PKCS8,
        encryption_algorithm=serialization.NoEncryption()
    )

    cert_pem = cert.public_bytes(serialization.Encoding.PEM)
    
    return private_key_pem.decode('utf-8'), cert_pem.decode('utf-8')
def check(timetamp=1,nouce=1,ID=1):
    print("无重放攻击，非双重支付，交易允许")
def sign_message(private_key_pem, message):
    private_key = serialization.load_pem_private_key(
        private_key_pem.encode(),
        password=None,
        backend=default_backend()
    )
    signature = private_key.sign(
        message.encode(),
        asym_padding.PSS(
            mgf=asym_padding.MGF1(hashes.SHA256()),
            salt_length=asym_padding.PSS.MAX_LENGTH
        ),
        hashes.SHA256()
    )
    return base64.b64encode(signature).decode('utf-8')
def verify_signature(public_key_pem, message, signature):
    public_key = serialization.load_pem_public_key(
        public_key_pem.encode(),
        backend=default_backend()
    )
    try:
        public_key.verify(
            base64.b64decode(signature),
            message.encode(),
            asym_padding.PSS(
                mgf=asym_padding.MGF1(hashes.SHA256()),
                salt_length=asym_padding.PSS.MAX_LENGTH
            ),
            hashes.SHA256()
        )
        return True
    except Exception as e:
        return False
def extract_public_key_from_cert(cert_pem):
    cert = x509.load_pem_x509_certificate(cert_pem.encode(), default_backend())
    public_key = cert.public_key()
    public_key_pem = public_key.public_bytes(
        encoding=serialization.Encoding.PEM,
        format=serialization.PublicFormat.SubjectPublicKeyInfo
    )
    return public_key_pem.decode('utf-8')
def hash_message(message):
    hashed = hashlib.sha256(message.encode()).hexdigest()
    return hashed
def encrypt_rsa(public_key_pem, plaintext):
    # 加载公钥
    public_key = serialization.load_pem_public_key(
        public_key_pem,
        backend=default_backend()
    )
    # 加密数据
    ciphertext = public_key.encrypt(
        plaintext,
        asym_padding.OAEP(
            mgf=asym_padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(),
            label=None
        )
    )
    return ciphertext
def generate_temp_aes_key():
    aes_key = hashlib.sha256(os.urandom(32)).digest()[:16]  # 生成随机的32字节并截取16字节作为AES密钥
    return aes_key
def encrypt_message(pk, message):
    cipher = AES.new(pk, AES.MODE_CBC)
    ct_bytes = cipher.encrypt(pad(message, AES.block_size))
    iv = base64.b64encode(cipher.iv).decode('utf-8')
    ct = base64.b64encode(ct_bytes).decode('utf-8')
    return iv + ct
def decrypt_rsa(private_key_pem, ciphertext):
    # 加载私钥
    private_key = serialization.load_pem_private_key(
        private_key_pem,
        password=None,
        backend=default_backend()
    )
    # 解密数据
    plaintext = private_key.decrypt(
        ciphertext,
        asym_padding.OAEP(
            mgf=asym_padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(),
            label=None
        )
    )
    return plaintext
def decrypt_message(pk, encrypted_message):
    iv = base64.b64decode(encrypted_message[:24])
    ct = base64.b64decode(encrypted_message[24:])
    cipher = AES.new(pk, AES.MODE_CBC, iv)
    pt = unpad(cipher.decrypt(ct), AES.block_size)
    return pt
privte_key_c='''-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----'''
cert_c='''-----BEGIN CERTIFICATE-----

BdkezHmFOINRw41Oy2uWrCVa
-----END CERTIFICATE-----'''
public_key_c=extract_public_key_from_cert(cert_c)
private_key_p='''-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----'''
cert_p='''-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'''
public_key_p=extract_public_key_from_cert(cert_p)
private_key_m='''-----BEGIN PRIVATE KEY-----

jptlsxt+DCDDu3m/9xO0ynyp
-----END PRIVATE KEY-----'''
cert_m='''-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'''
public_key_m=extract_public_key_from_cert(cert_m)

HOST='localhost'
PORT=5001

server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.bind((HOST, PORT))
server_socket.listen(5)
print("等待商家连接...")
client_socket, addr = server_socket.accept()
print("商家已连接：", addr)
start=time.time()
recv1=client_socket.recv(644).decode('utf-8')
recv2=client_socket.recv(496).decode('utf-8')
recv3=client_socket.recv(256)
recv4=client_socket.recv(600)
# print(recv3)
# print(recv4)

print("支付网关解密账户信息")
acc_sk_1=decrypt_rsa(private_key_p.encode(),recv3)
acc=acc_sk_1[:11]
sk_1=acc_sk_1[11:]
print("账户信息：",acc)
sig=decrypt_message(sk_1,recv4)
print("解密签名")

PI=sig[-19:].decode('utf-8')
timetamp,nouce,ID=PI[:2],PI[2:4],PI[4:6]
HASH_OI=sig[-19-64:-19].decode()
AuthREQ_sign=sig[:-19-64].decode('utf-8')
print("得到支付信息PI:",PI)
print("得到订单信息的哈希值HASH_OI:",HASH_OI)
print("得到支付授权的签名AuthREQ_sign")
print('正在进行参数检测……')
check(timetamp,nouce,ID)

print("\033[1;35mstep7:验证支付授权请求\033[0m")
if verify_signature(public_key_c,hash_message(HASH_OI+hash_message(PI)),AuthREQ_sign):
    print("验证签名成功")
else:
    print("验证签名失败")

HOST2 = 'localhost'
PORT2 = 5002
client_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
client_socket2.connect((HOST2, PORT2))
print("连接银行服务器成功")

print("支付网关向银行发送认证请求")
print("\033[1;35mstep8:请求支付授权\033[0m")
payment_clearing_request_to_issuer = "Payment clearing request to Issuer"
print('payment_clearing_request_to_issuer:',payment_clearing_request_to_issuer)

client_socket2.send(payment_clearing_request_to_issuer.encode('utf-8'))
print("\033[1;35mstep9:支付授权回应\033[0m")
AuthRES="Payment authorized"
print('AuthRES:',AuthRES)
print()
sk_3=generate_temp_aes_key()
sd1=encrypt_rsa(public_key_m.encode(),sk_3)
sig1=sign_message(private_key_p,hash_message(AuthRES))+AuthRES
enc1=encrypt_message(sk_3,sig1.encode())
sk_4=generate_temp_aes_key()
sd2=encrypt_rsa(public_key_p.encode(),sk_4)
CapTok="Capture token"
sig2=sign_message(private_key_p,hash_message(CapTok))+CapTok
enc2=encrypt_message(sk_4,sig2.encode())
client_socket.sendall(sd1)
client_socket.sendall(enc1.encode())
client_socket.sendall(sd2)
client_socket.sendall(enc2.encode())
print("支付网关给商家发送支付授权成功")

sd5=client_socket.recv(256)
enc5=client_socket.recv(517)
#支付网关解密sk_5
sk_5=decrypt_rsa(private_key_p.encode(),sd5)
print("解密出sk_5:",sk_5)
dec5=decrypt_message(sk_5,enc5)
print("得到CapREQ:",dec5.decode('utf-8')[-19:])
print("验证CapREQ签名")
sig5=dec5.decode('utf-8')[:-19]
print("\033[1;35mstep14:验证请求请款请求\033[0m")
if verify_signature(public_key_m,hash_message(dec5.decode('utf-8')[-19:]),sig5):
    print("验证成功")
else:
    print("验证失败")
    
#支付网关向银行发送PayREQ请求
print("\033[1;35mstep15:发送支付请款请求给银行\033[0m")
PayREQ="Payment request"
print('PayREQ:',PayREQ)
print()

client_socket2.send(PayREQ.encode('utf-8'))

#支付网关接收银行的PayRES并转发给商家
sd6=client_socket2.recv(256)
enc6=client_socket2.recv(516)
client_socket.sendall(sd6)
client_socket.sendall(enc6)

end=time.time()
#print("总耗时：",end-start)